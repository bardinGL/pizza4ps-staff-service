...
namespace Pizza4Ps.StaffService.Domain.Services
{
    public class StaffService : DomainService, IStaffService
    {
        ...
        public async Task<Guid> CreateAsync(string code, string name, string phone, string email, StaffTypeEnum type, Guid roleId)
        {
            var entity = new Staff(Guid.NewGuid(), code, name, phone, email, type, roleId);
        ...
        public async Task<Guid> UpdateAsync(Guid id, string code, string name, string phone, string email, StaffTypeEnum type, Guid roleId)
        {
            var entity = await _staffRepository.GetSingleByIdAsync(id);
            entity.UpdateStaff(code, name, phone, email, type, roleId);
            await _unitOfWork.SaveChangeAsync();
            return entity.Id;
        }
    }

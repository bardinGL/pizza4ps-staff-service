...
{
	public class IndividualScheduleService : DomainService, IIndividualScheduleService
	{
		private readonly IUnitOfWork _unitOfWork;
		private readonly IIndividualScheduleRepository _individualScheduleRepository;

		public IndividualScheduleService(IUnitOfWork unitOfWork, IIndividualScheduleRepository individualScheduleRepository)
		{
		...
		{
			var entity = new IndividualSchedule(Guid.NewGuid(), schedualDate, shiftStart, shiftEnd, status, staffId);
			_individualScheduleRepository.Add(entity);
		...
		{
			var entities = await _individualScheduleRepository.GetListAsTracking(x => ids.Contains(x.Id)).IgnoreQueryFilters().ToListAsync();
			if (entities == null) throw new ServerException(ServerErrorConstants.NOT_FOUND);
			foreach (var entity in entities)
			{
				if (IsHardDeleted)
				{
					_individualScheduleRepository.HardDelete(entity);
				}
				else
				{
					_individualScheduleRepository.SoftDelete(entity);
				}
			}
			await _unitOfWork.SaveChangeAsync();
		}
		...
		public async Task RestoreAsync(List<Guid> ids)
		{
			...
			foreach (var entity in entities)
			{
				_individualScheduleRepository.Restore(entity);
			}
			await _unitOfWork.SaveChangeAsync();
		}
		...
		{
			var entity = await _individualScheduleRepository.GetSingleByIdAsync(id);
			entity.UpdateIndividualSchedule(schedualDate, shiftStart, shiftEnd, status, staffId);
			await _unitOfWork.SaveChangeAsync();
			return entity.Id;
		}
	}
